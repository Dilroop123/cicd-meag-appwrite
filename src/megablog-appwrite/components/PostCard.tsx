    ...
    return (
        <Link to={`/post/${$id}`} style={{ textDecoration: 'none', listStyle: 'none', color: 'gray' }}>
            <div style={{ width: '100%', height: isGrid ? '275px' : '320px', border: '1px solid rgba(0, 0, 0, 0.25)', display: 'flex', marginTop: '25px' }}>
                <div style={{ flex: isGrid ? 2 : 1, backgroundColor: 'pink' }}>
                ...
                <div style={{ padding: '40px', flex: 2 }}>
                    ...
                    <div>
                        <span className="ellipsis_3_line" style={{ fontSize: isGrid ? '15px' : '17px' }}>{parse(content)}</span>
                    </div>


                    <div style={{ borderBottom: '1px solid rgba(0, 0, 0, 0.25)', marginTop: isGrid ? '25px' : '70px' }} />
                </div>
